    ...
    layui.use('table', function(){
        ...
        tableIns=table.render({
            ...
            ,cols: [[
                ...
                ,{field:'picClass', title:'图片分类'}
                ,{field:'title', title:'标题'}
                ,{field:'content', title: '内容'}
                ,{field:'imgUrls', title: '图片', minWidth:80}
                ,{field:'createTime', title: '创建时间'}
                ,{field:'state', title:'是否在职',width:95,align:'center',templet:'#state'}
...
function setJobUser(obj,id,nameVersion,checked){
    ...
    var isState=checked ? 0 : 1;
    var state=checked ? "生效":"失效";
    //是否离职
    layer.confirm('您确定要把标题尾：'+name+'设置为'+state+'状态吗？', {
        btn: ['确认','返回'] //按钮
    }, function(){
        $.post("/user/setJobUser",{"id":id,"job":isState,"version":version},function(data){
            if(isLogin(data)){
                if(data=="ok"){
                    //回调弹框
